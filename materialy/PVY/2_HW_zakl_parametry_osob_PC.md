...
Základní deska umožňuje _zapojení CPU (procesor), RAM (operační paměť), GPU (grafická karta), HDD/SSD_ a dalších. Na základní desce je také uložený BIOS.
...
##### BIOS/UEFI
BIOS a jeho vylepšená verze UEFI _zajišťují vazbu mezi operačním systémem a hardware_. V dnešní době se BIOS/UEFI používá při startu počítače pro inicializaci a konfiguraci připojeného HW. BIOS/UEFI je uložen na základní desce.
...
- Kapacita je množství dat, které lze na _pevný disk uložit_. Kapacity se nyní pohybuje v řádech TB.
